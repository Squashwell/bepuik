...
static int pose_bepuik_set_bone_exec(bContext *C, wmOperator *op)
{
//	Scene *scene = CTX_data_scene(C);
	Object *ob = BKE_object_pose_armature_get(CTX_data_active_object(C));
	bPoseChannel * pchan;
	
	int use_bepuik = RNA_boolean_get(op->ptr,"use_bepuik");
	if(RNA_boolean_get(op->ptr, "only_selected"))
	{
		CTX_DATA_BEGIN (C, bPoseChannel *, pchan, selected_pose_bones)
		{
			if(use_bepuik)
				pchan->bepuikflag |= BONE_BEPUIK;
			else
				pchan->bepuikflag &= ~BONE_BEPUIK;
		}
		CTX_DATA_END;
...
void POSE_OT_bepuik_set_bone(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Set BEPUik";
	ot->idname = "POSE_OT_bepuik_set_bone";
	ot->description = "Set BEPUik state on bones";
	
	/* callbacks */
	ot->exec = pose_bepuik_set_bone_exec;
	ot->poll = ED_operator_posemode;
	...
	RNA_def_boolean(ot->srna, "use_bepuik", TRUE, "BEPUik", "Use BEPUik");
}
